...
def train(args):
    ...
    # all_aspect_ids = torch.tensor([f.aspect_id for f in train_features], dtype=torch.long)
    ...
    if args.do_valid:
        ...
        # valid_all_aspect_ids = torch.tensor([f.aspect_id for f in train_features], dtype=torch.long)
...
def test(args, model):  # Load a trained model that you have fine-tuned (we assume evaluate on cpu)    
    ...
    all_aspect_ids = torch.tensor([f.aspect_id for f in eval_features], dtype=torch.long)

    eval_data = TensorDataset(all_input_ids, all_segment_ids, all_input_mask, all_label_ids, all_aspect_ids)
